...
setwd("~/tbPrevalenceReviewLMIC/tb-lmic-prevalence-review/")
...
# mfRatio <- model2_est0[1:(dim(model2_est0)[1]/2),]
# 
# mfRatio$value <- model2_est0 %>% filter(Sex == "Male") %>% dplyr::select(value) /
#                  model2_est0 %>% filter(Sex == "Female") %>% dplyr::select(value)

# mfRatio$Sex <- "mfRatio"

# model2_est <- rbind(model2_est0, mfRatio)
